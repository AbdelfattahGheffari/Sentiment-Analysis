...
from keras.layers import Dense ,Dropout , Embedding , Bi_LSTM, Bidirectional,SpatialDropout1D
import codecs
...
#Design Neural Network Architecture with Bi_LSTM 

Bi_LSTM_model = Sequential()

Bi_LSTM_model.add(Embedding(vocab_size, embidding_dim,input_length =max_length))

Bi_LSTM_model.add(Dropout(dropout_rate))

Bi_Bi_LSTM.add(Bidirectional(Bi_LSTM(8)))

Bi_LSTM_model.add(Dropout(dropout_rate))

Bi_LSTM_model.add(Dense(num_classes,activation='sigmoid'))

Bi_LSTM_model.compile(loss = 'binary_crossentropy', optimizer='adam',metrics=['accuracy',report.f1_m,report.recall_m,report.precision_m])
...
#history
history=Bi_LSTM_model.fit(X_train,  target_train, epochs =1, batch_size=batch_size,validation_data=(X_val,target_val),callbacks=[es])


loss, acc ,F1,recall,precision = Bi_LSTM_model.evaluate(X_test, target_test, batch_size=batch_size)
